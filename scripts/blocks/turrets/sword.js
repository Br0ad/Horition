...
collidesTiles:false,
  ...
  draw(e) {
    ...
    Draw.color(Pal.lancerLaser, Color.cyan, Pal.lancerLaser, e.fin())
    Lines.lineAngle(e.x, e.y, e.rotation(), 200)
    Tmp.v1.trns(e.rotation() + 180, 1);
    Damage.collideLine(e, e.team, this.hitEffect, e.x, e.y, e.rotation(), 200);
...
const sword = extendContent(PowerTurret, "sword", {})

sword.buildType = ent => {
  ent = extendContent(PowerTurret.PowerTurretBuild, sword, {
    BulletLife: 0,
    vec: new Vec2(),
    B1: null,
    B2: null,
    updateTile() {
      ...
      if (this.isShooting() === true && this.hasAmmo() === true && this.BulletLife > 0) {
       ...
       this.B2.rotation(this.rotation + 180);
    ...
    bullet(type, angle) {
      ...
      this.B2 = type.create(this, this.team, this.x + this.vec.x, this.y + this.vec.y, angle + 180);
      this.BulletLife = 300
